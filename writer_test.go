...
func TestWriter_Write(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expected: expected{
				bytes:   []byte{0x07, 0x06, 0x66, 0x6f, 0x6f, 0x62, 0x61, 0x72},
				offsets: []uint64{0},
			},
		},
		{
			...
			expected: expected{
				bytes:   []byte{0x08, 0x03, 0x06, 0x66, 0x6f, 0x6f, 0x62, 0x61, 0x72, 0x06, 0x62, 0x61, 0x7a, 0x62, 0x61, 0x72, 0x09, 0x77, 0x68, 0x61, 0x74, 0x20, 0x74, 0x68, 0x65, 0x3f},
				offsets: []uint64{0},
			},
		},
		{
		...
		},
		{
			...
			expected: expected{
				bytes:   []byte{0x07, 0x03, 0x66, 0x6f, 0x6f, 0x07, 0x03, 0x62, 0x61, 0x72, 0x02, 0xae, 0x01, 0x01, 0x01, 0x02, 0x07, 0x04, 0x40, 0x58, 0xc7, 0x0a, 0x3d, 0x70, 0xa3, 0xd7},
...
func TestWriter_WriteMap(t *testing.T) {
	...
	buf := new(bytes.Buffer)
	w := NewWriterFromBuffer(buf)
	offset, i, err := w.Write(m)
	assert.NilError(t, err)
	assert.Equal(t, offset, uint64(0))
	assert.Equal(t, i, 32)

	r := NewReaderFromBuffer(buf.Bytes())
	val, dt, err := r.Read(KeyValueMap)
	assert.NilError(t, err)
	assert.Equal(t, dt, KeyValueMap)
	assert.DeepEqual(t, val, m)
}
